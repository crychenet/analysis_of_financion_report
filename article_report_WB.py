...
def get_article_report(df, df_cost_of_goods):
    ...
    def get_fact_price_sales(column):
        fact_price = np.sum(df[(df['Артикул поставщика'] == column) &
                               (df['Обоснование для оплаты'].isin(sales_filter))]['Сумма фактических продаж'])
        return fact_price

    def get_fact_price_returns(column):
        fact_price = np.sum(df[(df['Артикул поставщика'] == column) &
                               (df['Обоснование для оплаты'].isin(return_filter))]['Сумма фактических продаж'])
        return fact_price
    ...
    def get_commission_WB_sales(column):
        commission_WB = np.sum(df[(df['Артикул поставщика'] == column) &
                                  (df['Обоснование для оплаты'].isin(sales_filter))]['Размер кВВ, руб'])
        return commission_WB

    def get_commission_WB_returns(column):
        commission_WB = np.sum(df[(df['Артикул поставщика'] == column) &
                                  (df['Обоснование для оплаты'].isin(return_filter))]['Размер кВВ, руб'])
        return commission_WB
